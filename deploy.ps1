...
gnome-terminal -- bash -c "sshpass -p 'root' ssh root@10.2.1.4; exec bash"
gnome-terminal -- bash -c "sshpass -p 'root' ssh root@10.2.1.4; exec bash"
gnome-terminal -- bash -c "sshpass -p 'root' ssh root@10.2.1.5; exec bash"

# Step 4: On both VMs, head into /root/project and execute make, then execute the respective erl commands
sshpass -p 'root' ssh root@10.2.1.4 << 'EOF'
cd /root/project
make
erl -pa ebin -name node1@10.2.1.4 -setcookie pastry -eval "controller:start(\"control1\", 10, \"node1@10.2.1.4\")."
EOF

sshpass -p 'root' ssh root@10.2.1.5 << 'EOF'
cd /root/project
make
